...
from models import Recommender, PPR
from metrics import get_ndcg, get_calibrated_recall
...
class Evaluator:
    ...
    def _loadData(self, train_path: str, test_path: str):
        ...
        self.item_popularity = self.train_df['item_id'].value_counts(
            normalize=True)
        self.user_activity = self.train_df.groupby('user_id')[
            'item_id'].count()
        self.playtime_stats = self.train_df.groupby(
            'item_id')['playtime'].agg(['mean', 'std'])
    ...
    def createValidationSplit(self, train_df: pd.DataFrame, val_size: float = 0.1, seed: int = 42) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        test_user_activity = self.test_in_df.groupby('user_id')[
            'item_id'].count()
        test_item_distr = self.test_in_df['item_id'].value_counts(
            normalize=True)
        test_playtime_distr = self.test_in_df.groupby('user_id')[
            'playtime'].mean()
        ...
        for user, group in train_df.groupby('user_id'):
            ...
            activity_diff = abs(
                user_activity - test_user_activity.mean()) / test_user_activity.std()
            ...
            if common_items:
                item_score = np.mean([min(user_items[i], test_item_distr[i]) / max(user_items[i], test_item_distr[i])
            ...
            playtime_diff = abs(
                user_playtime - test_playtime_distr.mean()) / test_playtime_distr.std()
            playtime_score = 1 / (1 + playtime_diff)

            # Combine scores with weights
            user_scores[user] = (0.4 * activity_score +
                                 0.4 * item_score +
                                 0.2 * playtime_score)
    ...
    def createFoldInSplit(self, val_df: pd.DataFrame, seed: int = 42) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        # Calculate test set characteristics
        test_items_per_user = self.test_in_df.groupby('user_id')[
            'item_id'].count()
        ...
        for _, group in val_df.groupby('user_id'):
            ...
            target_items = max(1, min(
                int(np.random.normal(
                    test_items_per_user.mean(),
                    test_items_per_user.std()
                )),
            ...
            for _, row in group.iterrows():
                ...
                playtime_score = 1 / \
                    (1 + abs(row['playtime'] - playtime_stats['mean']
                             ) / (playtime_stats['std'] + 1e-6))
    ...
    def createSampledDataset(self, df: pd.DataFrame, n_users: int, seed: int = 42) -> pd.DataFrame:
        ...
        activity_bins = pd.qcut(user_counts, q=5, labels=[
                                'v_low', 'low', 'med', 'high', 'v_high'])
        ...
        for bin_label in activity_bins.unique():
            ...
            sampled_users.extend(np.random.choice(
                bin_users, size=n_bin, replace=False))

        return df[df['user_id'].isin(sampled_users)]

    def _evaluateRecommendations(self, recommendations: Dict[int, List[int]], ground_truth: Dict[int, List[int]], k: int = 20) -> Dict[str, float]:
        """
    ...
    def runEvaluation(self, recommender: Recommender, sample_size: int = None, n_trials: int = 3) -> Tuple[Dict[str, float], Dict[str, float]]:
        """
        ...
        for trial in tqdm(range(n_trials), desc="Evaluation trials"):
            # Sample if requested
            if sample_size:
                train_data = self.createSampledDataset(
                    self.train_df,
                    sample_size,
                    seed=42 + trial
                )
            ...
            # Create splits
            train_split, val_split = self.createValidationSplit(
                train_data,
                seed=42 + trial
            )

            val_in, val_out = self.createFoldInSplit(
                val_split,
                seed=42 + trial
            )
            ...
            metrics = self._evaluateRecommendations(
        ...
        std_metrics = {
            metric: np.std([m[metric] for m in metrics_list])
            for metric in metrics_list[0].keys()
        }
    ...
    def createSubmission(self, recommender: Recommender, output_dir: str = "output/submissions") -> Tuple[str, str]:
        ...
        self.logger.info(f"Using recommender: {
                         recommender.__class__.__name__}")
        ...
        self.logger.info(f"Recommendations generated in {
        ...
        csv_path = f"{
            output_dir}/submission_{recommender.__class__.__name__}_{timestamp}.csv"
        zip_path = f"{
            output_dir}/submission_{recommender.__class__.__name__}_{timestamp}.zip"
...
if __name__ == "__main__":
    ...
    from models import MultiAlphaPPR
    print(evaluator.runEvaluation(MultiAlphaPPR(
        **params_mappr), sample_size=5000, n_trials=3))
    '''
    params_tppr = {
